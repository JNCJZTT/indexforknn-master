...
//        bag.remove(this.VertexName);                                    //去掉关键字
    ...
    //获得祖先节点的下标
    public int GetAncestorIndex(Vertex v){
        return this.TreeLevel-v.TreeLevel;
    }

    //构造KTNN
    public void BuildKTNN(){
        this.setActive();
        if(!IsBuildAncestor) {
            this.BuildAncestor();
        }
        //如果是活跃点 肯定已经构造过Ancestor 和 BuildKTNN
        //不是活跃点，可能之前构造过

        for(int i=0;i<Ancestors.size();i++){
            //遍历所有祖先节点
            Vnode Ancestor=Ancestors.get(i);
            if(Ancestor.dis!=-1){
                //如果到祖先是连通的,则更新祖先的KTNN
                AllVertices.get(Ancestor.name).UpdateKTNN(this.VertexName,Ancestor.dis);
            }
        }
    }
    ...
    public void displayKTNN(){
        for(int i=0;i<this.KTNN.size();i++){
            System.out.print(this.KTNN.get(i).name+" dis="+this.KTNN.get(i).dis+"  ");
        }
        System.out.println();
    }
