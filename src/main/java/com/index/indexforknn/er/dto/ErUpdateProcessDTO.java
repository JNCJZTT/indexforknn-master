package com.index.indexforknn.er.dto;
...
public class ErUpdateProcessDTO {
    ...
    public Set<Integer> mergeChangedActive() {
        Set<Integer> changedActiveSet = change2ActiveSet.stream()
                .filter(name -> ErVariable.INSTANCE.getVertex(name).isActive()).collect(Collectors.toSet());
