...
@Setter
@Slf4j
public class ErKnn extends DijkstraKnn<ErVertex> {
    ...
    public ErKnn(int queryName) {
        ...
        activeNodes.clear();
        nearestVertex = ErVariable.INSTANCE.getVertex(queryName);
        findGird();
    }
...
//                ErVariable.INSTANCE.getVertex(i).getCars();
//            }
            A(i);
        }
...
//        }
    ...
    public void findGird(){
        String clusterName = ErVariable.INSTANCE.getVertex(queryName).getClusterName();
        ...
        collect.clear();
        activeNodes.clear();
        activeNodes.addAll(ErVariable.INSTANCE.getCluster(clusterName).getActiveNode());
...
//                activeNodes.addAll(ErVariable.INSTANCE.getCluster(String.valueOf(addCluName)).getActiveNode());
//            }
...
//            }
            ...
            while (!borderList.isEmpty()&&activeNodes.size()<GlobalVariable.K){
                ...
                for (int j:ErVariable.INSTANCE.getVertex(i).getBorderGNames()){
                    if (ErVariable.INSTANCE.getCluster(String.valueOf(j)).visited){
                        continue;
                    }
                    ErVariable.INSTANCE.getCluster(String.valueOf(j)).visited=true;
                    activeNodes.addAll(ErVariable.INSTANCE.getCluster(String.valueOf(j)).getActiveNode());

                    collect.addAll(ErVariable.INSTANCE.getCluster(String.valueOf(j)).getBorderLink());
                }
                borderList.remove(0);
            }
...
//                    activeNodes.addAll(ErVariable.INSTANCE.getCluster(String.valueOf(j)).getActiveNode());
//                    collect.addAll(ErVariable.INSTANCE.getCluster(String.valueOf(j)).getBorderLink());
//                    carNumber+=ErVariable.INSTANCE.getCluster(String.valueOf(j)).getCarNum();
//                }
//                borderList.remove(0);
//            }
...
//        }
    ...
    private int eDis(int queryName, int iop){
        double qx=ErVariable.INSTANCE.getVertex(queryName).getX(),
                qy=ErVariable.INSTANCE.getVertex(queryName).getY(),
                iopx=ErVariable.INSTANCE.getVertex(iop).getX(),
                iopy=ErVariable.INSTANCE.getVertex(iop).getY();
...
//        Arrays.fill(queryArray, -1);
//        queryArray[queryName] = 0;
//        nearestVertex = ErVariable.INSTANCE.getVertex(nearestName);
        while (true){
            if (terminateEarly()) {
